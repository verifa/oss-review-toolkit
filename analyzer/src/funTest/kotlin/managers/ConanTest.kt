...
class ConanTest : WordSpec() {
   ...
   init {
        "Projects dependencies are detected correctly from conanfile.txt" {
            val packageFile = File(projectsDirTxt, "conanfile.txt")
            val vcsPath = vcsDirTxt.getPathToRoot(projectsDirTxt)
            val expectedResult = patchExpectedResult(
                File(projectsDirTxt.parentFile.parentFile, "conan-expected-output-txt.yml"),
        ...
        "Projects dependencies are detected correctly from conanfile.py" {
            val packageFile = File(projectsDirPy, "conanfile.py")
            val vcsPath = vcsDirPy.getPathToRoot(projectsDirPy)
            val expectedResult = patchExpectedResult(
                File(projectsDirPy.parentFile.parentFile, "conan-expected-output-py.yml"),
